...
export default function DraftCreateLayout() {
  ...
  return (
    <main className="flex md:flex-row flex-col min-h-screen w-full bg-blue-50">
      <Sidebar />
      <div className="flex flex-col w-full md:p-16 p-8">
        <h1 className="font-heading place-self-start text-5xl font-bold pb-8 text-black">
          Buat Draf Berita
        </h1>

        <div className="flex flex-col py-10 md:px-12 px-4 items-start self-stretch border-2 border-blue-400 rounded-md gap-5 bg-white">
          <div className="flex flex-col gap-5">
            <p className="font-body font-bold text-xl text-black">
              Buat draf berita secara otomatis dengan bantuan AI!<br></br>
              Cukup pilih model AI yang diinginkan dan masukkan deskripsi berita
              yang ingin dibuat.
            </p>
          ...
          <div className="flex flex-row self-stretch border-2 rounded-lg overflow-hidden">
            <div className="flex font-body text-md items-center bg-gray-100 px-5">
              Model
            </div>
            <select
              id="draft-save-options"
              value={selectedModel}
              onChange={(selected) => setSelectedModel(selected.target.value)}
